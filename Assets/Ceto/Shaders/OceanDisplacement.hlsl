...
{
	clip(0.5 - saturate(SAMPLE_TEXTURE2D_LOD(Ceto_Overlay_ClipMap, sampler_Ceto_Overlay_ClipMap, st.xy, 0).r));
}

/*
* Convert the world pos to projector space.
...
*/
float4 tex2DGrad(TEXTURE2D_PARAM(tex, texSampler), float2 uv, float2 dx, float2 dy)
{
	float2 px = Ceto_MapSize.x * dx;
    float2 py = Ceto_MapSize.x * dy;
	float lod = 0.5 * log2(max(dot(px, px), dot(py, py)));
	return SAMPLE_TEXTURE2D_LOD(tex, texSampler, uv, lod);
}
...
float3 SampleDisplacement(float2 uv, float2 dux, float2 duy)
{
	...
	float3 d = float3(0,0,0);
		...
		d = tex2DGrad(TEXTURE2D_ARGS(Ceto_DisplacementMap0, sampler_Ceto_DisplacementMap0), uv * igridSizes.x, dux * igridSizes.x, duy * igridSizes.x).xyz;
		d.xz *= Ceto_Choppyness.x;
		h += d;

		d = tex2DGrad(TEXTURE2D_ARGS(Ceto_DisplacementMap1, sampler_Ceto_DisplacementMap1), uv * igridSizes.y, dux * igridSizes.y, duy * igridSizes.y).xyz;
		d.xz *= Ceto_Choppyness.y;
		h += d;
	
		#ifdef CETO_USE_4_SPECTRUM_GRIDS
		d = tex2DGrad(TEXTURE2D_ARGS(Ceto_DisplacementMap2, sampler_Ceto_DisplacementMap2), uv * igridSizes.z, dux * igridSizes.z, duy * igridSizes.z).xyz;
		d.xz *= Ceto_Choppyness.z;
		h += d;

		d = tex2DGrad(TEXTURE2D_ARGS(Ceto_DisplacementMap3, sampler_Ceto_DisplacementMap3), uv * igridSizes.w, dux * igridSizes.w, duy * igridSizes.w).xyz;
...
half2 SampleSlope(float2 uv, half2 dux, half2 duy)
{
	half2 slope = half2(0,0);
		...
		slope += SAMPLE_TEXTURE2D(Ceto_SlopeMap0, sampler_Ceto_SlopeMap0, uv * igridSizes.x).xy;
		slope += SAMPLE_TEXTURE2D(Ceto_SlopeMap0, sampler_Ceto_SlopeMap0, uv * igridSizes.y).zw;
	
		#ifdef CETO_USE_4_SPECTRUM_GRIDS
			slope += SAMPLE_TEXTURE2D(Ceto_SlopeMap1, sampler_Ceto_SlopeMap1, uv * igridSizes.z).xy;
			slope += SAMPLE_TEXTURE2D(Ceto_SlopeMap1, sampler_Ceto_SlopeMap1, uv * igridSizes.w).zw;
		#endif
	#endif
	#endif
...
/*
* Sample the four grid slope maps - multiple outputs
*/
void SampleSlope(float2 uv, half2 dux, half2 duy, out half2 slope1, out half2 slope2, out half2 slope3)
{
	...
	slope3 = half2(0, 0);
		...
		s += SAMPLE_TEXTURE2D(Ceto_SlopeMap0, sampler_Ceto_SlopeMap0, uv * igridSizes.x).xy;
		s += SAMPLE_TEXTURE2D(Ceto_SlopeMap0, sampler_Ceto_SlopeMap0, uv * igridSizes.y).zw;
		slope1 = s;
	
		#ifdef CETO_USE_4_SPECTRUM_GRIDS
			s += SAMPLE_TEXTURE2D(Ceto_SlopeMap1, sampler_Ceto_SlopeMap1, uv * igridSizes.z).xy;
		#endif
		slope2 = s;
	
		#ifdef CETO_USE_4_SPECTRUM_GRIDS
			s += SAMPLE_TEXTURE2D(Ceto_SlopeMap1, sampler_Ceto_SlopeMap1, uv * igridSizes.w).zw;
		#endif
		slope3 = s;
	#endif
	#endif
...
/*
* Sample slope without derivatives
*/
half2 SampleSlopeSimple(float2 uv)
{
	half2 slope = half2(0,0);
...
/*
* Sample slope - multiple outputs without derivatives
*/
void SampleSlopeSimple(float2 uv, out half2 slope1, out half2 slope2, out half2 slope3)
{
	half2 s = half2(0,0);
	slope1 = half2(0, 0);
	slope2 = half2(0, 0);
...
/*
* Sample the four grid foam maps with derivatives
*/
fixed SampleFoam(float2 uv, half2 dux, half2 duy)
{
	...
	fixed jacobiBreak = 0;
		...
		jacobiBreak += SAMPLE_TEXTURE2D(Ceto_FoamMap0, sampler_Ceto_FoamMap0, uv * igridSizes.x).x;
		jacobiBreak += SAMPLE_TEXTURE2D(Ceto_FoamMap0, sampler_Ceto_FoamMap0, uv * igridSizes.y).y;
	
		#ifdef CETO_USE_4_SPECTRUM_GRIDS
			jacobiBreak += SAMPLE_TEXTURE2D(Ceto_FoamMap0, sampler_Ceto_FoamMap0, uv * igridSizes.z).z;
			jacobiBreak += SAMPLE_TEXTURE2D(Ceto_FoamMap0, sampler_Ceto_FoamMap0, uv * igridSizes.w).w;
		#endif
	#endif
	#endif
...
/*
* Sample foam without derivatives
*/
fixed SampleFoamSimple(float2 uv)
{
	uv += Ceto_PosOffset.xz;
...
void OceanPositionAndDisplacement(float4 uv, out float4 oceanPos, out float3 displacement)
{
	...
	float2 height = float2(0, 0);
		...
		height = SAMPLE_TEXTURE2D_LOD(Ceto_Overlay_HeightMap, sampler_Ceto_Overlay_HeightMap, st, 0).xy;
	#endif
	...
	float fade = length((cameraPos - oceanPos.xyz) * float3(1,0,1)) * 0.80 * unity_CameraProjection._m11;
...
void OceanNormalAndFoam(float4 uv, float4 st, float3 worldPos, out half3 norm, out half3 unmaskedNorm, out fixed4 foam)
{
	float4 oceanPos = OceanPos(uv);
		...
		half2 slope = SampleSlopeSimple(oceanPos.xz);
		norm = SlopeToWorldNormal(slope);
   	
   		foam = fixed4(0,0,0,0);
   		foam.x = SampleFoamSimple(oceanPos.xz);
	#else
	...
	#ifndef CETO_DISABLE_HEIGHT_OVERLAYS
		onorm = SAMPLE_TEXTURE2D_LOD(Ceto_Overlay_NormalMap, sampler_Ceto_Overlay_NormalMap, st.xy, 0);
	#endif

	fixed4 ofoam = fixed4(0, 0, 0, 0);
	#ifndef CETO_DISABLE_FOAM_OVERLAYS
		ofoam = SAMPLE_TEXTURE2D_LOD(Ceto_Overlay_FoamMap, sampler_Ceto_Overlay_FoamMap, st.xy, 0);
	#endif
...
void OceanNormalAndFoam(float4 uv, float4 st, float3 worldPos, out half3 norm1, out half3 norm2, out half3 norm3, out half3 unmaskedNorm2, out fixed4 foam)
{
	float4 oceanPos = OceanPos(uv);
		...
		SampleSlopeSimple(oceanPos.xz, slope1, slope2, slope3);
