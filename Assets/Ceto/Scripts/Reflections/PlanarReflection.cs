...
namespace Ceto
{
	...
	public class PlanarReflection : OceanComponent
    {
        ...
        void RenderReflection(Camera reflectionCam, RenderTexture target, Vector3 position, Quaternion rotation, Matrix4x4 projection, OceanCameraSettings settings)
        {
            ...
            // URP Fix: Wrap Camera.Render() in try-catch to prevent crashes
            try
            {
                #if UNITY_2019_1_OR_NEWER && !UNITY_EDITOR
                // In URP, Camera.Render() can cause issues. Use UniversalRenderPipeline.RenderSingleCamera if available
                UnityEngine.Rendering.Universal.UniversalRenderPipeline.RenderSingleCamera(
                    UnityEngine.Rendering.RenderPipelineManager.currentPipeline as UnityEngine.Rendering.Universal.UniversalRenderPipeline,
                    UnityEngine.Rendering.ScriptableRenderContext.EmptyContext,
                    reflectionCam
                );
                #else
            ...
            {
                // Silently catch URP camera data errors
                // Reflections will be disabled but won't crash
                Debug.LogWarning("URP Camera Data Error capturado. Las reflexiones serán deshabilitadas. Excepción: " + ex.Message);
            }
