...
namespace Ceto
{
	...
	public class PlanarReflection : OceanComponent
    {
        ...
        void RenderReflection(Camera reflectionCam, RenderTexture target, Vector3 position, Quaternion rotation, Matrix4x4 projection, OceanCameraSettings settings)
        {
            ...
            // URP: Nested camera renders with UniversalRenderPipeline.Render trigger
            // "Type UniversalCameraData has already been created". Skip planar
            // reflections under URP to avoid this crash. If reflections are
            // required, a URP-compatible reflection pass must be implemented.
            var rpAsset = UnityEngine.Rendering.GraphicsSettings.currentRenderPipeline;
            if (rpAsset != null && rpAsset.GetType().Name.Contains("UniversalRenderPipeline"))
            {
                return;
            }
